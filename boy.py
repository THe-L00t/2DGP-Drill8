from pico2d import load_image, get_time
from sdl2 import SDL_KEYDOWN, SDLK_SPACE, SDLK_RIGHT, SDL_KEYUP, SDLK_LEFT, SDLK_a

from state_machine import StateMachine

def space_down(eve):
    return eve[0] =='INPUT' and eve[1].type == SDL_KEYDOWN and eve[1].key == SDLK_SPACE

def time_out(eve):
    return eve[0] == 'TIME_OUT'

def right_down(eve):
    return eve[0] =='INPUT' and eve[1].type == SDL_KEYDOWN and eve[1].key == SDLK_RIGHT
def right_up(eve):
    return eve[0] =='INPUT' and eve[1].type == SDL_KEYUP and eve[1].key == SDLK_RIGHT
def left_down(eve):
    return eve[0] =='INPUT' and eve[1].type == SDL_KEYDOWN and eve[1].key == SDLK_LEFT
def left_up(eve):
    return eve[0] =='INPUT' and eve[1].type == SDL_KEYUP and eve[1].key == SDLK_LEFT
def a_down(eve):
    return eve[0] == 'INPUT' and eve[1].type == SDL_KEYDOWN and eve[1].key == SDLK_a

class AutoRun:

    def __init__(self, boy):
        self.boy = boy

    def enter(self,eve):
        self.boy.dir = 1
        self.boy.face_dir = 1
        self.boy.speed = 5
        self.boy.size = 100
        self.boy.auto_start_time = get_time()

    def exit(self,eve):
        pass

    def do(self):
        self.boy.frame = (self.boy.frame + 1) % 8
        self.boy.speed += 0.5
        self.boy.size += 5
        self.boy.y += 1.7
        if self.boy.x > 760:
            self.boy.dir = -1
            self.boy.face_dir = -1
        elif self.boy.x < 40:
            self.boy.dir = 1
            self.boy.face_dir = 1
        self.boy.x += self.boy.dir * self.boy.speed
        if get_time() - self.boy.auto_start_time > 5.0:
            self.boy.state_machine.handle_state_event(('TIME_OUT',Idle))

    def draw(self):
        if self.boy.face_dir == 1: # right
            self.boy.image.clip_draw(self.boy.frame * 100, 100, 100, 100, self.boy.x, self.boy.y,self.boy.size,self.boy.size)
        else: # face_dir == -1: # left
            self.boy.image.clip_draw(self.boy.frame * 100, 000, 100, 100, self.boy.x, self.boy.y,self.boy.size,self.boy.size)

class Run:

    def __init__(self, boy):
        self.boy = boy


    def enter(self,eve):
        self.boy.y = 90
        if left_down(eve):
            self.boy.dir = -1
            self.boy.face_dir = -1
        elif right_down(eve):
            self.boy.dir = 1
            self.boy.face_dir = 1
        elif left_up(eve):
            self.boy.dir = 1
            self.boy.face_dir = 1
        elif right_up(eve):
            self.boy.dir = -1
            self.boy.face_dir = -1

    def exit(self,eve):
        pass

    def do(self):
        self.boy.frame = (self.boy.frame + 1) % 8
        self.boy.x += self.boy.dir * 5

    def draw(self):
        if self.boy.face_dir == 1: # right
            self.boy.image.clip_draw(self.boy.frame * 100, 100, 100, 100, self.boy.x, self.boy.y,100,100)
        else: # face_dir == -1: # left
            self.boy.image.clip_draw(self.boy.frame * 100, 000, 100, 100, self.boy.x, self.boy.y,100,100)

class Sleep:

    def __init__(self, boy):
        self.boy = boy


    def enter(self,eve):
        self.boy.dir = 0

    def exit(self,eve):
        pass

    def do(self):
        self.boy.frame = (self.boy.frame + 1) % 8

    def draw(self):
        if self.boy.face_dir == 1: # right
            self.boy.image.clip_composite_draw(self.boy.frame * 100, 300, 100, 100, 3.141592 / 2,'', self.boy.x, self.boy.y-40,100,100)
        else: # face_dir == -1: # left
            self.boy.image.clip_composite_draw(self.boy.frame * 100, 200, 100, 100, -3.141592 / 2,'', self.boy.x, self.boy.y-40,100,100)

class Idle:

    def __init__(self, boy):
        self.boy = boy

    def enter(self,eve):
        self.boy.y = 90
        self.boy.dir = 0
        self.boy.wait_start_time = get_time()

    def exit(self,eve):
        pass

    def do(self):
        self.boy.frame = (self.boy.frame + 1) % 8
        if get_time() - self.boy.wait_start_time > 5.0:
            self.boy.state_machine.handle_state_event(('TIME_OUT',None))

    def draw(self):
        if self.boy.face_dir == 1: # right
            self.boy.image.clip_draw(self.boy.frame * 100, 300, 100, 100, self.boy.x, self.boy.y,100,100)
        else: # face_dir == -1: # left
            self.boy.image.clip_draw(self.boy.frame * 100, 200, 100, 100, self.boy.x, self.boy.y,100,100)


class Boy:
    def __init__(self):
        self.x, self.y = 400, 90
        self.frame = 0
        self.face_dir = 1
        self.dir = 0
        self.image = load_image('animation_sheet.png')

        self.IDLE = Idle(self)
        self.SLEEP = Sleep(self)
        self.RUN = Run(self)
        self.AUTO = AutoRun(self)
        self.state_machine = StateMachine(
            self.IDLE,
            {
                self.SLEEP : {space_down : self.IDLE},
                self.IDLE : {time_out : self.SLEEP, right_down : self.RUN, left_down : self.RUN,left_up : self.RUN, right_up : self.RUN, a_down:self.AUTO},
                self.RUN : {right_up : self.IDLE, left_up : self.IDLE, right_down : self.IDLE, left_down : self.IDLE},
                self.AUTO : {right_down : self.RUN, left_down:self.RUN, time_out:self.IDLE}
            }
        )

    def update(self):
        self.state_machine.update()


    def draw(self):
        self.state_machine.draw()

    def handle_event(self, event):
        self.state_machine.handle_state_event(('INPUT', event))
        pass
